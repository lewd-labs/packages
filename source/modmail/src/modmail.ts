    import {
			...
			ModMailModelOptions,
			ModMailOptions,
		} from "../typings/modmail.interface"
		...
		export class ModMailClient {
			...
			public async modmailListener(message: Message) {
				...
				if (message.channel.type === "DM") {
					...
					if (!channel) return this.model.deleteMany({ Channel: data.Channel })
			...
			public async deleteMail({ channel, reason }: CloseMailSessionOptions) {
				...
				if (data) {
					...
					if (this.options.transcriptChannel) {
						const transcriptChannel = this.options.client.channels.cache.get(
