...
class GiveawayClient {
    schema = mongoose_1.default.model("GuildGiveAwayModel");
    options;
     ...
     */
        ...
        this.schema.findOne({ MessageID, Activated: true }, async (err, data) => {
            ...
            if (err)
                throw err;
            if (!data)
                throw new Error("There are no giveaways currently running with " + MessageID + " id");
            if (getWinner) {
    ...
    getCurrentGiveaways(activatedOnly = true, all = false, message) {
        return new Promise((ful, rej) => {
            if (all) {
                ...
                if (activatedOnly) {
                    ful(this.collection.filter((value) => value.Guild === message.guild.id && value.Activated === true));
                }
